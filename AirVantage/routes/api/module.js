...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (!module) {
      res.status(400).json({
        msg: 'There is no module in database'
      });
    }
...
router.get('/user/:userID', auth, async (req, res) => {
  try {
    ...
    if (!module) {
      res.status(400).json({
        msg: 'There is no module profile for this user'
      });
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    const { name, serialnumber, imei, macaddress } = req.body;
    ...
    try {
      let checkModuleName = await Module.findOne({
        name: moduleFields.name
      });
      let checkModuleSerialNumber = await Module.findOne({
        serialnumber: moduleFields.serialnumber
      });
      if (checkModuleName || checkModuleSerialNumber) {
        return res.status(400).json({
          msg: 'This module is available, please enter new module'
        });
      }
